...
для Chrome и Яндекс Браузера, а при сборке с сертификатом разработчика — и для Safari: оно
добавляет кнопку «Скачать» на сайтах и отправляет ролик в приложение. Скачивает приложение через yt-dlp и ffmpeg, а там, где yt-dlp
...
2. Внизу окна приложения нажать «Включить» у строки «Расширение для Safari выключено»:
   откроются Safari → Настройки → Расширения. Поставить галочку у «Загрузка видео».
3. Разрешить расширению работать на сайтах (при первом нажатии на значок или
   «Изменить сайты…» в тех же настройках).

Код расширения общий с Chrome (`chrome-extension/`), обновляется вместе с приложением.
...
- **Расширение** (`chrome-extension/`, Manifest V3). Для Safari тот же код вшивается
  в приложение как `Contents/PlugIns/VideoLoader Safari.appex`:
...
| `chrome-extension/` | Расширение для Chrome, Яндекс Браузера и Safari |
