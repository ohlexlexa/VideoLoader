// Работает в контексте самой страницы Instagram и Threads (world: MAIN) с самого начала загрузки.
// У Threads тот же формат постов, что у Instagram. Лента, профили и переходы между постами
// подгружаются фоновыми запросами (GraphQL, /api/v1/), а не лежат в HTML. Скрипт подсматривает ответы этих запросов и запоминает посты по коду
